...
from middlewared.api.current import (SPDKHostCreateArgs,
...
class SPDKHostModel(sa.Model):
    __tablename__ = 'services_spdk_host'

    id = sa.Column(sa.Integer(), primary_key=True)
    spdk_host_hostnqn = sa.Column(sa.String(255), unique=True)
    spdk_host_dhchap_key = sa.Column(sa.EncryptedText(), nullable=True)
    spdk_host_dhchap_ctrl_key = sa.Column(sa.EncryptedText(), nullable=True)
    spdk_host_dhchap_dhgroup = sa.Column(sa.Integer(), default=0)
    spdk_host_dhchap_hash = sa.Column(sa.Integer(), default=0)


class SPDKHostService(CRUDService):

    class Config:
        namespace = 'spdk.host'
        datastore = 'services.spdk_host'
        datastore_prefix = 'spdk_host_'
        datastore_extend = 'spdk.host.extend'
        cli_private = True
    ...
    async def do_create(self, data):
        ...
        await self.__validate(verrors, data, 'spdk_host_create')
        verrors.check()

        await self.compress(data)
        with open("/data/hostcompress.json","w") as f:
            json.dump(data,f,indent=4)
        data['id'] = await self.middleware.call(
            'datastore.insert', self._config.datastore, data,
            {'prefix': self._config.datastore_prefix})
        added=await self.get_instance(data['id'])
        with open("/data/hostadded.json","w") as f:
    ...
    async def do_update(self, audit_callback, id_, data):
        ...
        verrors = ValidationErrors()
        await self.__validate(verrors, new, 'spdk_host_update', old=old)
        verrors.check()
    ...
    async def do_delete(self, audit_callback, id_, options):
        ...
        host_subsys_ids = {x['id']: x['subsys']['name'] for x in
                           await self.middleware.call('spdk.host_subsys.query', [['host.id', '=', id_]])}
        if host_subsys_ids:
            if force:
                await self.middleware.call('spdk.host_subsys.delete_ids', list(host_subsys_ids))
            else:
                count = len(host_subsys_ids)
                if count == 1:
                    name = list(host_subsys_ids.values())[0]
                    verrors.add('spdk_host_delete.id',
    ...
    @api_method(SPDKHostGenerateKeyArgs, SPDKHostGenerateKeyResult, roles=['SHARING_NVME_TARGET_WRITE'])
    ...
    @api_method(SPDKHostDhchapDhgroupChoicesArgs, SPDKHostDhchapDhgroupChoicesResult)
    ...
    @api_method(SPDKHostDhchapHashChoicesArgs, SPDKHostDhchapHashChoicesResult)
