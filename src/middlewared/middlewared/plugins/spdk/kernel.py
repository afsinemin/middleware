...
#                         SPDK_KERNEL_CONFIG_DIR,
#                         SPDK_NODE_A_ANA_GRPID,
#                         SPDK_NODE_B_ANA_GRPID,
#                         PORT_ADDR_FAMILY,
#                         PORT_TRTYPE)
#
...
#         parent_dir = pathlib.Path(SPDK_KERNEL_CONFIG_DIR, self.directory)
...
#             path = pathlib.Path(SPDK_KERNEL_CONFIG_DIR, self.directory, key)
...
#
#
# class SPDKHostConfig(SPDKConfig):
#     directory = 'hosts'
#     query = 'spdk.host.query'
#     query_key = 'hostnqn'
#
...
#
#
# class SPDKPortConfig(SPDKConfig):
#     directory = 'ports'
#     query = 'spdk.port.query'
#     query_key = 'index'
#
#     def config_dict(self, render_ctx):
...
#         config = {}
#         non_ana_port_ids = render_ctx['spdk.port.usage']['non_ana_port_ids']
#         ana_port_ids = render_ctx['spdk.port.usage']['ana_port_ids']
...
#                         choices = render_ctx[f'{prefix}.spdk.port.transport_address_choices']
...
#         match render_ctx['failover.node']:
#             case 'A':
#                 return pathlib.Path(path, 'ana_groups', str(SPDK_NODE_A_ANA_GRPID))
#             case 'B':
#                 return pathlib.Path(path, 'ana_groups', str(SPDK_NODE_B_ANA_GRPID))
#             case _:
#                 return None
#
...
#         if render_ctx['spdk.global.ana_active']:
...
#
#
# class SPDKPortReferralConfig(SPDKConfig):
#     directory = 'ports'
#     query = 'spdk.port.usage'
...
#         return {self.index(port['index']): port for port in render_ctx['spdk.port.query']}
#
...
#         port_id_to_index = {port['id']: self.index(port['index']) for port in render_ctx['spdk.port.query']}
...
#
#         for portpath in pathlib.Path(SPDK_KERNEL_CONFIG_DIR, 'ports').iterdir():
#             try:
...
#
#
# class SPDKPortAnaReferralConfig(SPDKPortReferralConfig):
#     directory = 'ports'
#     query = 'spdk.port.usage'
...
#         result = {}
#         for port in render_ctx['spdk.port.query']:
...
#
#
# class SPDKSubsysConfig(SPDKConfig):
#     directory = 'subsystems'
#     query = 'spdk.subsys.query'
#     query_key = 'subnqn'
#
#     def pre_delete(self, path: pathlib.Path, render_ctx: dict):
...
#
#         result['attr_model'] = render_ctx['spdk.subsys.model']
#         result['attr_firmware'] = render_ctx['spdk.subsys.firmware']
#         return result
#
#
# class SPDKNamespaceConfig(SPDKConfig):
#     directory = 'namespaces'
#     query = 'spdk.namespace.query'
#     query_key = 'nsid'
#
#     @contextmanager
#     def render(self, render_ctx: dict):
...
#         for subsys in render_ctx['spdk.subsys.query']:
#             subsys_id = subsys['id']
...
#             parent_dir = pathlib.Path(SPDK_KERNEL_CONFIG_DIR,
#                                       'subsystems',
...
#                 do_ana = subsys_ana
#             else:
#                 do_ana = bool(render_ctx['spdk.global.ana_enabled'])
#
#         if do_ana:
#             match render_ctx['failover.node']:
#                 case 'A':
#                     result['ana_grpid'] = SPDK_NODE_A_ANA_GRPID
#                 case 'B':
#                     result['ana_grpid'] = SPDK_NODE_B_ANA_GRPID
#
...
#         rootdir = pathlib.Path(SPDK_KERNEL_CONFIG_DIR, self.src_parentdir)
#         dstdir = pathlib.Path(SPDK_KERNEL_CONFIG_DIR, self.dst_dir)
#         to_unlink = []
...
#
#
# class SPDKHostSubsysConfig(SPDKLinkConfig):
#     query = 'spdk.host_subsys.query'
...
#
#
# class SPDKPortSubsysConfig(SPDKLinkConfig):
#     query = 'spdk.port_subsys.query'
#     src_parentdir = 'ports'
...
#                 index = raw_index
#             case _:
#                 if render_ctx['spdk.global.ana_enabled']:
#                     index = raw_index + ANA_PORT_INDEX_OFFSET
#                 else:
#                     index = raw_index
#
...
#     # for subsys in data['spdk.subsys.query']:
#     #     subsys_id = subsys['id']
#     #     subsys_dict = {
#     #         "subsys": subsys,
#     #         "namespaces": [],
...
#
#     # with open("/data/spdk.json", "w") as f:
...
#     # order of operations.
#     # with (
#     #     SPDKSubsysConfig().render(config),
#     #     SPDKHostConfig().render(config),
#     #     SPDKPortConfig().render(config),
#     #     SPDKPortReferralConfig().render(config),
#     #     SPDKPortAnaReferralConfig().render(config),
#     #     SPDKHostSubsysConfig().render(config),
#     #     SPDKPortSubsysConfig().render(config),
#     #     SPDKNamespaceConfig().render(config),
#     # ):
#     #     pass
#
#
...
#     try:
#         pathlib.Path(SPDK_KERNEL_CONFIG_DIR,
#                      'subsystems',
#                      subnqn,
#                      'namespaces',
#                      str(nsnum),
...
#
# def clear_config():
#     config_root = pathlib.Path(SPDK_KERNEL_CONFIG_DIR)
...
#
#
# def spdk_kernel_module_loaded():
#     return pathlib.Path(SPDK_KERNEL_CONFIG_DIR).is_dir()
#
#
