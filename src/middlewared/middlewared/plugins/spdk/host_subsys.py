...
from middlewared.api.current import (SPDKHostSubsysCreateArgs,
...
class SPDKHostSubsysModel(sa.Model):
    __tablename__ = 'services_spdk_host_subsys'

    id = sa.Column(sa.Integer(), primary_key=True)
    spdk_host_subsys_host_id = sa.Column(sa.ForeignKey('services_spdk_host.id'), index=True)
    spdk_host_subsys_subsys_id = sa.Column(sa.ForeignKey('services_spdk_subsys.id'), index=True)


class SPDKHostSubsysService(CRUDService):

    class Config:
        namespace = 'spdk.host_subsys'
        datastore = 'services.spdk_host_subsys'
        datastore_prefix = 'spdk_host_subsys_'
        datastore_extend_fk = ['host', 'subsys']
        cli_private = True
        role_prefix = 'SHARING_NVME_TARGET'
        entry = SPDKHostSubsysEntry
    ...
    async def do_create(self, data):
        ...
        await self.__validate(verrors, data, 'spdk_host_subsys_create')
        verrors.check()

        data['id'] = await self.middleware.call(
            'datastore.insert', self._config.datastore, data,
            {'prefix': self._config.datastore_prefix})
        added_data= await self.get_instance(data['id'])

        if await spdkServiceManager.status():

            if added_data["subsys"]["allow_any_host"]:
                await spdkSocketManager.set_subsystem_allow_any_host(added_data["subsys"]["subnqn"],
                                                                     True)
                await  spdkSocketManager.remove_all_hosts(added_data["subsys"]["subnqn"])
            else:
                await spdkSocketManager.set_subsystem_allow_any_host(added_data["subsys"]["subnqn"],
                                                                     False)
                await spdkSocketManager.subsystem_add_host(added_data["subsys"]["subnqn"],
                                                           added_data["host"]["hostnqn"],
                                                           added_data["host"]["dhchap_key"],
                                                           added_data["host"]["dhchap_ctrl_key"])
    ...
    async def do_update(self, audit_callback, id_, data):
        ...
        verrors = ValidationErrors()
        await self.__validate(verrors, new, 'spdk_host_subsys_update', old=old)
        verrors.check()
    ...
    async def delete_ids(self, to_remove):
        # This is called internally (from spdk.host.delete).  Does not require
    ...
    async def __validate(self, verrors, data, schema_name, old=None):
        ...
        try:
            await self.middleware.call('spdk.host.query', [['id', '=', host_id]], {'get': True})
