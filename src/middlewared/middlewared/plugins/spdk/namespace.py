...
from middlewared.api.current import (SPDKNamespaceCreateArgs,
...
class SPDKNamespaceModel(sa.Model):
    __tablename__ = 'services_spdk_namespace'
    __table_args__ = (
        sa.Index(
            'services_spdk_namespace_spdk_namespace_subsys_id__spdk_namespace_nsid_uniq',
            'spdk_namespace_subsys_id', 'spdk_namespace_nsid', unique=True
        ),
    )

    id = sa.Column(sa.Integer(), primary_key=True)
    spdk_namespace_nsid = sa.Column(sa.Integer())
    spdk_namespace_subsys_id = sa.Column(sa.ForeignKey('services_spdk_subsys.id'), index=True)
    spdk_namespace_device_type = sa.Column(sa.Integer())
    spdk_namespace_device_path = sa.Column(sa.String(255), unique=True)
    spdk_namespace_filesize = sa.Column(sa.Integer(), nullable=True)
    spdk_namespace_device_uuid = sa.Column(sa.String(40), unique=True)
    spdk_namespace_device_nguid = sa.Column(sa.String(40), unique=True)
    spdk_namespace_enabled = sa.Column(sa.Boolean())


class SPDKNamespaceService(SharingService):

    # For SharingService
    path_field = 'device_path'

    class Config:
        namespace = 'spdk.namespace'
        datastore = 'services.spdk_namespace'
        datastore_prefix = 'spdk_namespace_'
        datastore_extend = 'spdk.namespace.extend'
        datastore_extend_fk = ['subsys']
        cli_private = True
        role_prefix = 'SHARING_NVME_TARGET'
        entry = SPDKNamespaceEntry
    ...
    async def do_create(self, data):
        ...
        await self.__validate(verrors, data, 'spdk_namespace_create')
        verrors.check()

        await self.middleware.call('spdk.namespace.save_file', data, 'spdk_namespace_create', verrors)
        verrors.check()
        ...
        async with NSID_LOCK:
            ...
            addedNamespace= await self.get_instance(data["id"])

            subsystem = await self.middleware.call("spdk.subsys.get_subsystem_by_id", data['subsys_id'])
            if await SPDKServiceManager.status():
                await spdkSocketManager.add_namespace("/dev/"+data["device_path"], subsystem["subnqn"],data['nsid'],
                                                addedNamespace["device_uuid"],addedNamespace["device_nguid"])
    ...
    async def do_update(self, audit_callback, id_, data):
        ...
        verrors = ValidationErrors()
        await self.__validate(verrors, new, 'spdk_namespace_update', old=old)
        verrors.check()

        await self.middleware.call('spdk.namespace.save_file', new, 'spdk_namespace_update', verrors, old)
        verrors.check()
    ...
    async def do_delete(self, audit_callback, id_, options):
        ...
        if spdkServiceManager.status() and rv:
            await spdkSocketManager.delete_namespace(data["subsys"]["subnqn"],data["nsid"])
    ...
    async def delete_ids(self, to_remove):
        # This is called internally (from spdk.subsys.delete).  Does not require
                ...
                if old:
                    if _new_size := data.get('filesize'):
                        ...
                        if new_size > old_size:
                            ...
                            self.middleware.call_sync('spdk.namespace.resize_namespace', data['id'])
    ...
    async def __validate(self, verrors, data, schema_name, old=None):
        ...
        await self.middleware.call('spdk.namespace.clean_type_and_path', data, schema_name, verrors)
        ...
        for key in ('device_uuid', 'device_nguid'):
            data[key] = await self.__generate_uuid(device_path,data.get(key), key)

    @private
    async def __generate_uuid(self, device_path: str, old_uuid, key):
        NAMESPACE = uuid.NAMESPACE_DNS
        if old_uuid not in (None, ''):
            return old_uuid
        existing = [i[key] for i in (
            await self.middleware.call('spdk.namespace.query', [], {'select': [key]})
        )]
        for i in range(UUID_GENERATE_RETRIES):
            new_uuid = str(uuid.uuid5(NAMESPACE, device_path))
