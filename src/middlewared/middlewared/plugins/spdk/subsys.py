...
from middlewared.api.current import (SPDKSubsysCreateArgs,
...
class SPDKSubsysModel(sa.Model):
    __tablename__ = 'services_spdk_subsys'

    id = sa.Column(sa.Integer(), primary_key=True)
    spdk_subsys_name = sa.Column(sa.String(), nullable=False, unique=True)
    spdk_subsys_subnqn = sa.Column(sa.String(length=MAX_NQN_LEN), nullable=False, unique=True)
    spdk_subsys_serial = sa.Column(sa.String(), unique=True)
    spdk_subsys_allow_any_host = sa.Column(sa.Boolean(), default=False)
    spdk_subsys_pi_enable = sa.Column(sa.Boolean(), nullable=True, default=None)
    spdk_subsys_qid_max = sa.Column(sa.Integer(), nullable=True, default=None)
    spdk_subsys_ieee_oui = sa.Column(sa.Integer(), nullable=True, default=None)
    spdk_subsys_ana = sa.Column(sa.Boolean(), nullable=True, default=None)


class SPDKSubsysService(CRUDService, SPDKStandbyMixin):

    class Config:
        namespace = 'spdk.subsys'
        datastore = 'services.spdk_subsys'
        datastore_prefix = 'spdk_subsys_'
        datastore_extend_context = "spdk.subsys.extend_context"
        datastore_extend = "spdk.subsys.extend"
        cli_private = True
        role_prefix = 'SHARING_NVME_TARGET'
        entry = SPDKSubsysEntry
    ...
    async def do_create(self, data):
        ...
        See `spdk.host.create` and `spdk.host_subsys.create`.
        """
        verrors = ValidationErrors()
        await self.__validate(verrors, data, 'spdk_subsys_create')
        verrors.check()
        data['id'] = await self.middleware.call(
            'datastore.insert', self._config.datastore, data,
            {'prefix': self._config.datastore_prefix})
        addedData = await self.middleware.call(
            'datastore.query', self._config.datastore, [["id", "=", data['id']]],
        ...
        if await SPDKServiceManager.status():
            await spdkSocketManager.create_subsystem(addedData["subnqn"], addedData["allow_any_host"], addedData["serial"],ana_reporting=spdkConfigs["ana"])

        # with open("/data/create2.txt", "w") as f:
        #     json.dump(addedData, f, indent=4)
    ...
    async def do_update(self, audit_callback, id_, data):
        ...
        verrors = ValidationErrors()
        await self.__validate(verrors, new, 'spdk_subsys_update', old=old)
        verrors.check()
        ...
        updadeted=await self.get_instance(id_)
        if updadeted["allow_any_host"]:
            await spdkSocketManager.set_subsystem_allow_any_host(updadeted["subnqn"],
                                                                 True)
            await  spdkSocketManager.remove_all_hosts(updadeted["subnqn"])
        #await self.middleware.call('spdk.global.reload')
        return updadeted
    ...
    async def do_delete(self, audit_callback, id_, data):
        ...
        namespace_ids = {x['id']: x['nsid'] for x in
                         await self.middleware.call('spdk.namespace.query', [['subsys.id', '=', id_]])}
        if namespace_ids:
            if force:
                await self.middleware.call('spdk.namespace.delete_ids', list(namespace_ids))
            else:
                count = len(namespace_ids)
                if count == 1:
                    name = list(namespace_ids.values())[0]
                    verrors.add('spdk_subsys_delete.id',
        ...
        port_subsys_ids = [x['id'] for x in await self.middleware.call('spdk.port_subsys.query',
                                                                       [['subsys.id', '=', id_]],
                                                                       {'select': ['id']})]
        if port_subsys_ids:
            if force:
                await self.middleware.call('spdk.port_subsys.delete_ids', list(port_subsys_ids))
            else:
        ...
        host_subsys_ids = [x['id'] for x in await self.middleware.call('spdk.host_subsys.query',
                                                                       [['subsys.id', '=', id_]],
                                                                       {'select': ['id']})]
        if host_subsys_ids:
            await self.middleware.call('spdk.host_subsys.delete_ids', host_subsys_ids)
    ...
    async def extend_context(self, rows, extra):
        if extra.get('verbose'):
            return {
                EXTENDED_CONTEXT_KEY_HOSTS: await self.middleware.call('spdk.host_subsys.query'),
                EXTENDED_CONTEXT_KEY_NAMESPACES: await self.middleware.call('spdk.namespace.query'),
                EXTENDED_CONTEXT_KEY_PORTS: await self.middleware.call('spdk.port_subsys.query'),
            }
        return {}
    ...
    async def subsys_serial(self, serial):
        ...
        used_serials = [i['serial'] for i in (
            await self.middleware.call('spdk.subsys.query', [], {'select': ['serial']})
        )]
    ...
    @private
    async def get_subsystem_by_id(self, subsys_id):
        return await self.get_instance(subsys_id)

    @private
    async def get_subsystems(self):
    ...
    async def subsys_subnqn(self, subnqn, name):
        ...
        basenqn = (await self.middleware.call('spdk.global.config'))['basenqn']
