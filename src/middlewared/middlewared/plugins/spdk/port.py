...
from middlewared.api.current import (SPDKPortCreateArgs,
...
class SPDKPortModel(sa.Model):
    __tablename__ = 'services_spdk_port'

    id = sa.Column(sa.Integer(), primary_key=True)
    spdk_port_index = sa.Column(sa.Integer(), unique=True)
    spdk_port_addr_trtype = sa.Column(sa.Integer())
    # addr_trsvcid port number for IPv4 | IPv6, but string for AF_IB, None for FC
    spdk_port_addr_trsvcid = sa.Column(sa.String(255), nullable=True, default=None)
    spdk_port_addr_traddr = sa.Column(sa.String(255))
    spdk_port_addr_adrfam = sa.Column(sa.Integer())
    spdk_port_inline_data_size = sa.Column(sa.Integer(), nullable=True, default=None)
    spdk_port_max_queue_size = sa.Column(sa.Integer(), nullable=True, default=None)
    spdk_port_pi_enable = sa.Column(sa.Boolean(), nullable=True, default=None)
    spdk_port_enabled = sa.Column(sa.Boolean())


class SPDKPortService(CRUDService):

    class Config:
        namespace = 'spdk.port'
        datastore = 'services.spdk_port'
        datastore_prefix = 'spdk_port_'
        datastore_extend = 'spdk.port.extend'
        cli_private = True
    ...
    async def do_create(self, data):
        ...
        await self.__validate(verrors, data, 'spdk_port_create')
        verrors.check()
    ...
    @private
    async def get_port_by_id(self, id):
    ...
    async def do_update(self, audit_callback, id_, data):
        ...
        verrors = ValidationErrors()
        await self.__validate(verrors, new, 'spdk_port_update', old=old)
        verrors.check()
    ...
    async def do_delete(self, audit_callback, id_, options):
        ...
        port_subsys_ids = {x['id']: x['subsys']['name'] for x in
                           await self.middleware.call('spdk.port_subsys.query', [['port.id', '=', id_]])}
        if port_subsys_ids:
            if force:
                ...
                if count == 1:
                    ...
                    verrors.add('spdk_port_delete.id',
    ...
    async def has_active_ports(self) -> bool:
        if not await self.middleware.call('spdk.global.running'):
            return False

        # Get all non-disabled ports
        port_ids = [port['id'] for port in await self.middleware.call('spdk.port.query',
                                                                      [['enabled', '=', True]],
                                                                      {'select': ['id']})]

        if await self.middleware.call('spdk.port_subsys.query',
                                      [['port.id', 'in', port_ids]],
                                      {'count': True}):
            return True
        return False
    ...
    async def usage(self) -> dict:
        ...
        """
        all_ports = {port['id']: port for port in await self.middleware.call('spdk.port.query')}
        ...
        if not await self.middleware.call('failover.licensed'):
            ...
            ana_enabled = await self.middleware.call('spdk.global.ana_enabled')
            if ana_enabled:
            ...
            subsystems = {sub['id']: sub for sub in await self.middleware.call('spdk.subsys.query',
                                                                               [['ana', '!=', None]])}
            if subsystems:
                # It's complicated
                if ana_enabled:
                    for subsys_id, subsys in subsystems.items():
                        if not subsys['ana']:
                            ...
                            delta = {ps['port']['id'] for ps in
                                     await self.middleware.call('spdk.port_subsys.query',
                                                                [['subsys.id', '=', subsys_id]])}
        ...
        non_ana_referrals = []
        ana_referrals = []
        if (await self.middleware.call('spdk.global.config'))['xport_referral']:
    ...
    async def __validate(self, verrors, data, schema_name, old=None):
        ...
        try:
            existing = await self.middleware.call('spdk.port.query', filters, {'get': True})
        except MatchNotFound:
            existing = None
        # try:
        #     existing_nvmet = await self.middleware.call('nvmet.port.query', filters, {'get': True})
        # except MatchNotFound:
        #     existing_nvmet = None


        # if data['addr_trsvcid'] == 4420:
        #     verrors.add(f'{schema_name}.addr_trsvcid',
        #                 'You cant use  4420 port its only for nvmet')
        if old is None:
            # Create
            # Ensure that we're not duplicating an existing entry
            if existing:
                verrors.add(f'{schema_name}.addr_traddr','There already is a port using the same transport and address')

            # if existing_nvmet:
            #     verrors.add(f'{schema_name}.addr_traddr',
            #      'There already is a port using the same transport and address with nvmet')
        else:
            # Update.
            # Ensure that we're not duplicating an existing entry
            ...
            if await self.middleware.call('spdk.port_subsys.query',
                ...
                if old['enabled'] and await self.middleware.call('spdk.global.running'):
    ...
    @api_method(SPDKPortTransportAddressChoicesArgs, SPDKPortTransportAddressChoicesResult)
    async def transport_address_choices(self, addr_trtype, force_ana):
        ...
        match addr_trtype:
            case PORT_TRTYPE.TCP.api:
                if force_ana or (await self.middleware.call('spdk.global.config'))['ana']:
            ...
            case PORT_TRTYPE.RDMA.api:
                if not (await self.middleware.call('spdk.global.config'))['rdma']:
                    return choices
